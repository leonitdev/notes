"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.NoteRepository = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const mongoose_2 = require("mongoose");
const note_schema_1 = require("./schema/note.schema");
let NoteRepository = class NoteRepository {
    constructor(model) {
        this.model = model;
    }
    async getNotes(query) {
        try {
            return await this.model.find(query).exec();
        }
        catch (error) {
            throw new common_1.InternalServerErrorException(error);
        }
    }
    async createNote(body) {
        try {
            const note = new note_schema_1.Note(body);
            const item = new this.model(note);
            note.id = item._id;
            await item.save();
            return note;
        }
        catch (error) {
            throw new common_1.InternalServerErrorException(error);
        }
    }
    async getNoteById(id) {
        try {
            return await this.model.findOne({ _id: new mongoose_2.Types.ObjectId(id) }).exec();
        }
        catch (error) {
            throw new common_1.InternalServerErrorException(error);
        }
    }
    async updateNote(id, body) {
        try {
            return await this.model
                .findOneAndUpdate({ _id: new mongoose_2.Types.ObjectId(id) }, body, { new: true })
                .exec();
        }
        catch (error) {
            throw new common_1.InternalServerErrorException(error);
        }
    }
    async deleteNote(id) {
        try {
            return await this.model
                .findOneAndDelete({ _id: new mongoose_2.Types.ObjectId(id) })
                .exec();
        }
        catch (error) {
            throw new common_1.InternalServerErrorException(error);
        }
    }
    async deleteAllNotes() {
        try {
            return await this.model.deleteMany().exec();
        }
        catch (error) {
            throw new common_1.InternalServerErrorException(error);
        }
    }
};
NoteRepository = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, mongoose_1.InjectModel)(note_schema_1.Note.name)),
    __metadata("design:paramtypes", [mongoose_2.Model])
], NoteRepository);
exports.NoteRepository = NoteRepository;
//# sourceMappingURL=note.repository.js.map