...
const user_schema_1 = require("../../repositories/schema/user.schema");
...
let UserController = class UserController {
    ...
    async getUsers(res, req) {
        const items = await this.service.getUsers(req.query);
        const { status } = items;
        return res.status(status).json(items);
    }
    async createUser(res, body) {
        const item = await this.service.createUser(body);
        return res.status(201).json(item);
    }
};
__decorate([
    (0, swagger_1.ApiOkResponse)({ type: user_schema_1.User, isArray: true }),
    (0, common_1.Get)(),
